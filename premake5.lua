...
function check_raylib()
    if(os.isdir("vendor/raylib/raylib-source") == false) then
        if(not os.isfile("raylib-master.zip") and not os.isfile("raylib-source.zip") ) then
            print("Raylib not found, downloading from github")
            local result_str, response_code = http.download("https://github.com/raysan5/raylib/archive/refs/tags/" .. raylib_ver .. ".zip", "raylib.zip", {
                progress = download_progress,
                headers = { "From: Premake", "Referer: Premake" }
            })
        end
        print("Unzipping to " ..  os.getcwd())
        zip.extract("raylib.zip", os.getcwd() .. "/vendor/raylib")
        os.rename(os.getcwd() .. "/vendor/raylib/raylib-" .. raylib_ver, os.getcwd() .. "/vendor/raylib/raylib-source")
