...
void BattleUIPlayer::performAction(std::function<void(std::tuple<Player::Action, int>)> onPicked) {
	...
	else if (currentMenu == attackUI) {
		createAttackButtons();
	}
	else if (currentMenu == itemsUI) {
		createItemButtons();
	}
	isShown = true;
...
void BattleUIPlayer::createWeaponButtons() {
	Instattiate([&](Engine::Scene* scene) {
		...
		for (int i = 0; i < weapons.size(); i++) {
			...
			button.getComponent<Engine::Components::TransformComponent>().Position = { 0.0f, 70.0f + 70.0f * i };
...
void BattleUIPlayer::createAttackButtons() {
	Instattiate([&](Engine::Scene* scene) {
		Engine::Entity buttons[Attack::maxAttacks];

		auto battleMenu = scene->getEntityByName("attackMenu");

		if (!battleMenu.hasComponent<Engine::Components::ScriptComponent>())
			battleMenu.addComponent<Engine::Components::ScriptComponent>().bind<ButtonManager>();
		if (!battleMenu.hasComponent<Engine::Components::ButtonManagerData>())
			battleMenu.addComponent<Engine::Components::ButtonManagerData>();

		auto& uiMgrData = battleMenu.getComponent<Engine::Components::ButtonManagerData>();
...
void BattleUIPlayer::createItemButtons() {
	Instattiate([&](Engine::Scene* scene) {
		...
		auto& weapons = scene->m_GlobalStorage->get<Inventory>("inventory")->items;
		std::vector<Engine::Entity> buttons;
		int totalSkipped = 0;
		for (int i = 0; i < weapons.size(); i++) {
			auto& w = weapons[i];
			if (w.amout > 0) {
				std::string t = w.name + "  x" + std::to_string(w.amout);
				auto button = createDialogButton(t, "", i, scene);
				button.getComponent<Engine::Components::TransformComponent>().Position = { 0.0f, 70.0f + 70.0f * (i - totalSkipped) };
				button.getComponent<Engine::Components::BackgroundComponent>().scale = { 4.0, 4.0 };
...
void BattleUIPlayer::refreshUI() {
	if (currentMenu == weaponsUI) {
		...
		auto c = getEntityByName("attackList");
		...
		auto children = itemsUI.getChildren();
		for (int i = 1; i < children.size(); i++) {
			auto& ent = children[i];
			getSceneManager().getActveScene().removeEntity(ent);
			itemsUI.removeChild(ent);
		}
	}
}
